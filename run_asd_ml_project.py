...
Important note:
The miRNA expression dataset used here was already biologically preprocessed
and normalized by the original dataset authors. This script does not perform
raw sequencing preprocessing. It performs machine-learning preparation steps
such as variance filtering, scaling, feature selection, model training, and
evaluation.
...
def load_and_align_data(expression_path: Path, labels_path: Path):
    ...
    # Average duplicated miRNA IDs, then transpose so rows = samples and columns = miRNAs.
...
def make_qc_plot(X: pd.DataFrame, y: np.ndarray, outpath: Path):
    """
    PCA visualization only.
    This is not used to train or evaluate the classifier.
    Since the dataset is already biologically preprocessed, PCA is used only
    to visualize structure in the expression data.
    """
    X_qc = StandardScaler().fit_transform(X)
...
def main():
    ...
    X, y, labels, expr_raw = load_and_align_data(
    ...
    if missing_values_total > 0:
        raise ValueError(
            f"The dataset contains {missing_values_total} missing values. "
            "Because this script assumes the downloaded dataset is already "
            "preprocessed by the authors, please inspect the input files first."
        )
    ...
    qc = {
        ...
        "note": "Dataset was already biologically preprocessed/normalized by original authors.",
    }

    pd.Series(qc, dtype="object").to_csv(outdir / "data_qc_summary.csv")

    make_qc_plot(X, y, outdir / "pca_qc_plot.png")
    ...
    # The dataset was already biologically preprocessed by the original authors.
    # The following are ML preparation/modeling steps.
    # They are fit only on the training folds during grid search/CV to avoid leakage.
    pipeline = Pipeline([
    ...
    with open(outdir / "RESULTS_SUMMARY.md", "w", encoding="utf-8") as f:
        ...
        f.write(f"- Missing values: {missing_values_total}\n")
        f.write("- Dataset note: the expression data was already biologically preprocessed/normalized by the original authors.\n\n")

        f.write("## Leakage Prevention\n")
        f.write(
            "The dataset was already biologically preprocessed and normalized by the original data authors. "
            "In this project, ML preparation steps such as variance filtering, standardization, and feature selection "
            "were placed inside a scikit-learn Pipeline. These steps were fit only on the training data during "
            "grid search/CV and then applied to the held-out test data to avoid data leakage.\n\n"
        )
        ...
        f.write("\n\n## Important Caveat\n")
        f.write(
            "This is a small p >> n dataset, so the model is useful for a class project "
            "and exploratory biomarker screening, not for clinical diagnosis. Results should be "
